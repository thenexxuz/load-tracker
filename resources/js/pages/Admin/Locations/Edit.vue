...
const form = useForm({
  ...
  country: props.location.country || 'US',
  type: props.location.type,
  latitude: props.location.latitude,
  longitude: props.location.longitude,
  is_active: props.location.is_active,
  recycling_location_id: props.location.recycling_location_id,
  emails: props.location.emails ? props.location.emails.join(', ') : '', // ← pre-fill as comma-separated string
  expected_arrival_time: props.location.expected_arrival_time || null,
})
...
const submit = () => {
  form.put(route('admin.locations.update', props.location.id), {
    ...
    onError: (errors) => {
      ...
      Notify.failure('Please fix the errors below.')
    },
    onFinish: () => {
    ...
    <div class="p-6">
      ...
      >
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <!-- Recycling Location (only for DC) -->
        ...
        <div class="flex justify-end mt-8">
          <button
            ...
            class="bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-700 dark:hover:bg-indigo-800 text-white px-8 py-3 rounded-md font-medium transition-colors disabled:opacity-50"
          >
