...
it('keeps shipment show rates using viewer-specific carrier visibility rules', function (): void {
    ...
    expect($page)
        ...
        ->toContain("rate_destinations: rateDestinations = []")
        ->toContain("const rateNameFilter = ref('')")
        ->toContain("const rateCarrierFilter = ref('')")
        ->toContain("const rateDestinationFilter = ref('')")
        ->toContain('const filteredRates = computed(() => {')
        ->toContain('v-model="rateNameFilter"')
        ->toContain('v-model="rateCarrierFilter"')
        ->toContain('v-model="rateDestinationFilter"')
        ->toContain('v-for="rate in filteredRates"')
        ->toContain('Name')
